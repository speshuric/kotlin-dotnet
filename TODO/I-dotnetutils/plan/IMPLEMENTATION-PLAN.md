...
| I1 | **Типизированные хэндлы**: `TypeSystem/Handles.TypeSystem.cs` (30 struct'ов: 25 entity + 5 heap); companion-константы `Handle.{Module,Assembly}Definition`, `EntityHandle.{Module,Assembly}Definition`; хвост `MetadataFlags.cs` (HeapHandleType, StringHandleType, StringKind). Поправка 2: `HandleCollections.TypeSystem.cs` отсечён как reader-side (analysis/01 §9). **[DONE]** | ~2750 | I0 | факты из `HandleTests`, `TagToTokenTests` (подмножества) |
| I2 | Internal utils + Blob-ядро: `BitArithmetic`, `BlobUtilities`, `Hash`, `ExceptionUtilities`, `Blob`, `BlobBuilder`(+Enum), `ReservedBlob`, `BlobWriter`, `BlobWriterImpl`, `BlobContentId`. **[DONE]** | ~2900 | I1 (heap-хэндлы для BlobHandle/GuidHandle) | `BlobTests`, `BlobUtilitiesTests`, `BlobContentIdTests` |

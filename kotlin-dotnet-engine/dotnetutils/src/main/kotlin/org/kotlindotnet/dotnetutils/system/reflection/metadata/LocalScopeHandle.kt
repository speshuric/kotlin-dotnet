...
@JvmInline
value class LocalScopeHandle private constructor(
    ...
    companion object {
        private val className: String get() = "LocalScopeHandle"
        private val tokenType: UInt get() = TokenTypeIds.LOCAL_SCOPE
        private val tokenTypeSmall: UByte get() = HandleType.LOCAL_SCOPE.toUByte()

        internal fun fromRowId(rowId: Int) = LocalScopeHandle(rowId)

        internal fun fromHandle(handle: Handle): LocalScopeHandle {
            check(handle.vType == tokenTypeSmall) { "handle has wrong kind for $className" }
            return fromRowId(handle.rowId)
        }

        internal fun fromEntityHandle(entity: EntityHandle): LocalScopeHandle {
